...
# Business Appetite Data with SIC Codes, Eligibility, and Notes
business_appetite = {
    "Artisan Contractors": {
...
root = tk.Tk()
root.title("Powell Agency Business Appetite")
...
header = tk.Label(
    root,
    text="Powell Agency Business Appetite Checker",
...
frame1.pack(pady=10)
tk.Label(frame1, text="Select Business Type:", font=("Segoe UI", 12, "bold"), bg="#ffffff").pack(anchor="w")
business_type_menu = ttk.Combobox(
    frame1, textvariable=business_type_var,
    values=list(business_appetite.keys()), state="readonly", width=40
)
business_type_menu.pack()

# Subcategory Dropdown
frame2 = tk.Frame(root, bg="#ffffff")
frame2.pack(pady=10)
tk.Label(frame2, text="Select Subcategory:", font=("Segoe UI", 12, "bold"), bg="#ffffff").pack(anchor="w")
subcategory_menu = ttk.Combobox(frame2, textvariable=subcategory_var, values=[], state="readonly", width=40)
subcategory_menu.pack()

# Result Display
frame3 = tk.Frame(root, bg="#ffffff")
frame3.pack(pady=20)
result_label = tk.Label(
    frame3, text="", font=("Segoe UI", 11),
    bg="#ffffff", justify="left", wraplength=750, fg="#222"
)
result_label.pack()

# Update subcategories when business type is selected
def update_subcategories(event):
    selected = business_type_var.get()
    if selected in business_appetite:
        subcategory_menu['values'] = list(business_appetite[selected].keys())
        subcategory_var.set('')

business_type_menu.bind("<<ComboboxSelected>>", update_subcategories)

# Show eligibility info
def show_eligibility():
    b_type = business_type_var.get()
    sub_type = subcategory_var.get()
    if not b_type or not sub_type:
        result_label.config(text="⚠️ Please select both a business type and a subcategory.", fg="red")
        return
    data = business_appetite.get(b_type, {}).get(sub_type, {})
    text = (
        f"📌 {sub_type}\n"
        f"✅ Approval Status: {data.get('rating', 'N/A')}\n"
        f"📊 SIC Code: {data.get('SIC', 'N/A')}\n"
...
# Check Button
tk.Button(root, text="Check Eligibility", command=show_eligibility, bg="#4CAF50", fg="white", relief="flat").pack(pady=10)

# Footer
tk.Label(
    root,
    text="© 2025 Powell Agency | Built by Keith Coleman Jr",
